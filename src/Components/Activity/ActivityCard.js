...
export default function ActivityCard({id, title, date, setDeleteAlert, setDeletedId}){
  ...
  return (
    <>
      <div className="p-6 mr-5 mb-7 w-[235px] h-[235px] bg-white rounded-xl border border-gray-200 shadow-md" data-cy="activity-item">
        <Link to={`detail/${id}`}>
          <div className="activity-body h-[158px] cursor-ponter">
            <h5 className="mb-2 text-lg font-bold text-gray-900" data-cy="activity-item-title">{title}</h5>
          </div>
        </Link>
        <div className="activity-footer flex justify-between">
          <span className="text-[#888888] text-sm" data-cy="activity-item-date">{parseDate}</span>
